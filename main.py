...
DISTRIBUTION_STRATEGIES = {
    ...
    "Beta_.3_.3": partial(pes.distribution_strategies.BetaDistribution, a=0.3, b=0.3),
    "Beta_4_4": partial(pes.distribution_strategies.BetaDistribution, a=4, b=4),
    "Beta_4_1": partial(pes.distribution_strategies.BetaDistribution, a=4, b=1),
    "Beta_3_.3": partial(pes.distribution_strategies.BetaDistribution, a=3, b=0.3),
    "Beta_1_4": partial(pes.distribution_strategies.BetaDistribution, a=1, b=4),
    "Beta_.3_3": partial(pes.distribution_strategies.BetaDistribution, a=0.3, b=3),
}

VOTING_MECHANISMS = {
    # Candidate
    "InstantRunoffCandidate": pes.voting_mechanisms.candidate.InstantRunoffMechanism,
    "Median": pes.voting_mechanisms.candidate.MedianMechanism,
    "Plurality": pes.voting_mechanisms.candidate.PluralityMechanism,
    "WeightedInstantRunoffCandidate": pes.voting_mechanisms.candidate.WeightedInstantRunoffMechanism,
    # Average
    "InstantRunoffAverage": pes.voting_mechanisms.average.InstantRunoffMechanism,
    "Mean": pes.voting_mechanisms.average.MeanMechanism,
    "RankedChoice": pes.voting_mechanisms.average.RankedChoiceMechanism,
    "WeightedInstantRunoffAverage": pes.voting_mechanisms.average.WeightedInstantRunoffMechanism,
    "WeightlessAverageAll": pes.voting_mechanisms.average.WeightlessAverageAllMechanism,
}
...
def get_dataframe_from_files(
    dir_with_files: str, verbose: bool = False
) -> pd.DataFrame:
    ...
    for i, file in enumerate(data_files, start=1):
        if verbose and i % 100 == 0:
            log(f"Reading {i}/{len(files)} " f"({i / len(files) * 100:.2f}%). . .")
...
def output_df(df: pd.DataFrame, dir_: str):
    ...
    output_filename = f"PES" f"_{len(df)}_rows" f"_{now.strftime('%d-%m-%Y_%H-%M-%S')}"
...
def perform_iterations():
    ...
    paired_mechs = [
        (v, "NoOp") for v in set(VOTING_MECHANISMS) & set(WEIGHT_IGNORING_VMS)
    ] + [
        (v, w)
        for v in set(VOTING_MECHANISMS) - set(WEIGHT_IGNORING_VMS)
        for w in set(WEIGHTING_MECHANISMS) - {"NoOp"}
    ]
    non_vm_combos = (
        len(proxy_counts)
        * len(PROXY_DISTRIBUTIONS)
        * len(PROXY_EXTENTS)
        * len(inactive_counts)
        * len(INACTIVE_DISTRIBUTIONS)
        * len(INACTIVE_EXTENTS)
    )
    ...
    for i, (
        ...
        # Create system
        proxies = [
            pes.Agent(DISTRIBUTION_STRATEGIES[proxy_dist_strategy](), proxy_extent)
            for _ in range(num_proxies)
        ]
        inactive_voters = [
            pes.Agent(
                DISTRIBUTION_STRATEGIES[inactive_dist_strategy](), inactive_extent
            )
            for _ in range(num_inactive)
        ]
        inactive_voters = [
            pes.InactiveVoter(a, WEIGHTING_MECHANISMS[inactive_weighting_mech]())
...
def run_iterations(system: pes.ProxySystem, num_iterations: int) -> [int]:
    output = [system.estimate(0) for _ in range(num_iterations)]
    return output
